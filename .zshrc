# 26/06/2023 Shadow - Setup modern terminal on box with plugins and aliases
...
# # Backup unmodified screen command
# alias _original_screen=screen

# # screen command wrapper
# # Ensure screens with duplicate names cannot be started!
# screen() {
#     if [[ "$1" == "-S" || "$1" == "-s" ]] && _original_screen -list | grep -q "\.$2[[:space:]]"; then
#         echo -e "([Warning] A screen named $2 already exists!\n
#                   Currently Active Screens:
#                   $(_original_screen -ls)\n
#                   Issues that can cause duplicate screens:
#                   1) Multiple staff are on the box executing server start/stop commands at the same time.
#                   2) Server $2 is still in the process of saving and shutting down.
#                   3) Server $2 has failed to shutdown properly and has frozen.\n
#                   Fixes:
#                   If you believe server shutdown has entered a frozen state, force kill using:
#                   screen -X -S $2 kill)"
#         return 1 # Return Error Code
#     else
#         # Call the original screen command with all passed arguments
#         _original_screen "$@"
#     fi
# }
...
# Live - Bungeecord/Waterfall Proxy
#for cmd in proxy_on b_on bung_on bungee_on; do
#    alias $cmd="start_server bungee $mc_bung"
#done

#for cmd in proxy_off b_off bung_off bungee_off; do
#    alias $cmd="stop_server bungee $mc_bung"
#done

#for cmd in proxy_restart b_restart bung_restart bungee_restart; do
#    alias $cmd="bungee_off && bungee_on"
#done
...
# Bungee
#for cmd in log_b log_bung log_bungee bungee_log; do
#    alias $cmd="tail -F --line=10000 $mc_bungl/latest.log"
#done
# Hub
